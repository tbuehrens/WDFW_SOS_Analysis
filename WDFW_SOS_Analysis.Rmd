...
specialcaselist <- list(
  ...
  quote(
     !(COMMONPOPNAME2 == "North Fork Toutle River - winter Steelhead"&
      POPFITNOTES %in% c("Green River redd count expansion on MS and Tributary indexes","NF Toutle River Trap Count")
     )
  )
...
read_csv(here::here("year_pops.csv")) %>%
  full_join(read_csv("https://raw.githubusercontent.com/tbuehrens/WDFW_SOS_Analysis/refs/heads/2024/year_pops.csv"),
    by = join_by(ESU_DPS, COMMONPOPNAME2),
    suffix = c(".2026", ".2024")
  ) %>%
  mutate(Updates = ifelse(maxyr.2026 > maxyr.2024, "", "not updated")) %>%
